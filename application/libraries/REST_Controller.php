...
abstract class REST_Controller extends MY_Controller {
    ...
    public function __construct($config = 'rest')
    {
                ...
                ($this->config->item('allow_auth_and_keys') === TRUE && $this->_allow === TRUE)))
        {
            ...
            if ($this->config->item('rest_ip_whitelist_enabled') == 1)
            {
    ...
    private function get_local_config($config_file)
    {
        ...
        $rest_ip_whitelist_enabled = $this->config->item('rest_ip_whitelist_enabled');
        $rest_ip_whitelist = $this->config->item('rest_ip_whitelist');
        ...
        $this->config->set_item('rest_ip_whitelist_enabled', $rest_ip_whitelist_enabled);
        $this->config->set_item('rest_ip_whitelist', $rest_ip_whitelist);
    ...
    protected function _check_login($username = NULL, $password = FALSE)
    {
        ...
        if ( !$this->Users_model->verify($username, substr(do_hash($password), 0, 32), true) )
        {
            return FALSE;
        }
